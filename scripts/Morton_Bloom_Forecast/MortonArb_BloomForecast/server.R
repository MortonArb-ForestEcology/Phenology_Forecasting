...
dat.b <- read.csv(file.path(path.in , "Redbud_bloom_obs.csv"))

dat.ghcn$TMAX <- (dat.ghcn$TMAX * 9/5) + 32
dat.ghcn$TMEAN <- (dat.ghcn$TMEAN * 9/5) + 32
dat.ghcn$TMIN <- (dat.ghcn$TMIN * 9/5) + 32

# -------------------------------------
# Creating our bloom visualizations
# -------------------------------------
...
function(input, output) { 
  ...
  # observe({ 
        output$redbud <- renderPlot({
          ...
          dat.budsum <- read.csv(file.path(path.in, "bloom", paste0("Bloom_Prediciton_Summary_", input$`Forecast date` ,".csv")))
          ...
          dat.forecast[dat.forecast$VAR == "TMEAN", "min"] <- (dat.forecast[dat.forecast$VAR == "TMEAN", "min"]* 9/5) + 32
          dat.forecast[dat.forecast$VAR == "TMEAN", "mean"] <- (dat.forecast[dat.forecast$VAR == "TMEAN", "mean"]* 9/5) + 32
          dat.forecast[dat.forecast$VAR == "TMEAN", "max"] <- (dat.forecast[dat.forecast$VAR == "TMEAN", "max"]* 9/5) + 32
          
          #Taking the mean date of each year
          Year <- unique(dat.b$year)
          prev.date <- as.data.frame(Year)
          for(YR in unique(prev.date$Year)){
            prev.date[prev.date$Year == YR, "Mean.Date"] <- mean.Date(as.Date(format(as.Date(dat.b[dat.b$year == YR,"Date"]),"%m-%d"), format = "%m-%d"))
          }
          
          mean.yr <- lubridate::yday(mean.Date(prev.date$Mean.Date))
          ...
          plot.threshB <- ggplot(data=dat.ghcn) +
            ...
            theme_bw() +
            guides(fill="none") +
            guides(color=guide_legend(title= c(paste0(lubridate::year(Sys.Date()), " Weather"))))+
          ...
          Mean.d.temp <- ggplot(data=dat.ghcn) +
            ...
            scale_y_continuous(name="Mean Daily Temp (F)" ,expand=c(0,0)) +
            theme_bw() +
            guides(fill="none") +
            guides(color=guide_legend(title= c(paste0(lubridate::year(Sys.Date()), " Weather"))))+
            ggtitle(paste0("Mean Temp (F)"))+
            theme(legend.position = c(0.2, 0.75),
                  legend.background = element_blank(),
                  panel.grid.minor.x = element_blank(),
                  panel.grid.minor.y = element_blank()) +
            geom_rect(data=dat.budsum,
                      aes(xmin = lb, xmax=ub, ymin=-Inf, ymax=Inf), fill="deeppink1", alpha=0.5)+
            geom_vline(xintercept=mean.yr)+
          ...
          Cum.gdd5 <- plot.threshB + 
            ...
            dat.newdist <- read.csv(file.path(path.in, "bloom", paste0("Prop_Bloom_Prediction_", Sys.Date() ,".csv")))
            Prob.dist <- ggplot() + 
              geom_density(data=dat.newdist, aes(x=yday), fill = "deeppink1", alpha=0.5, show.legend = FALSE) +
              ...
              scale_y_continuous(name="Probability of Bloom" ,expand=c(0,0)) +
              theme_bw() +
              guides(fill="none") +
              ggtitle(paste0("Eastern redbud: Bloom DOY 75% C.I."))+
          ...
          title <- ggdraw() + 
            draw_label(
              paste0("Eastern redbud predicted bloom date range ", as.Date((dat.budsum$lb-1), origin = paste0(lubridate::year(Sys.Date()),"-01-01")) ,
                     " to ", as.Date((dat.budsum$ub-1), origin = paste0(lubridate::year(Sys.Date()),"-01-01"))),
              fontface = 'bold',
              x = 0,
              hjust = 0, 
              size = 20,
